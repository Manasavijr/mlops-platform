...
class DriftDetector:
    ...
    def check(self, current_scores: List[float]) -> DriftCheckResponse:
        ...
        if drift_detected:
            ...
            logger.info(f"No drift. KS={ks_stat:.4f} p={p_value:.4f}")
        return response

    def reference_stats(self) -> Dict[str, float]:
        if not self._stats_cache:
            self._stats_cache = {
                "mean": round(float(np.mean(self._reference)), 4),
                "std": round(float(np.std(self._reference)), 4),
                "median": round(float(np.median(self._reference)), 4),
                "min": round(float(np.min(self._reference)), 4),
                "max": round(float(np.max(self._reference)), 4),
