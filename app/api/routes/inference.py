...
async def predict(payload: InferenceRequest, registry=Depends(get_registry)):
    ...
    logger.info(f"Inference | request_id={request_id} n={len(predictions)} latency={latency_ms:.1f}ms")
    return InferenceResponse(predictions=predictions, model_name=registry.model_name, latency_ms=round(latency_ms, 2), request_id=request_id)
    ...
    try:
        ...
        with open(log_path, "a") as f:
            for pred in predictions:
                f.write(json.dumps({"request_id": request_id, "timestamp": datetime.utcnow().isoformat(), "label": pred.label, "score": pred.score, "model_version": pred.model_version}) + "\n")
    except Exception as e:
